...
### 16. 프로젝트 목표 설정: 플랫폼 만들기
장기 목표: 앞으로 만드는 프로젝트를 계속 올릴 수 있는 개인 플랫폼

### 17. JSON 파일로 데이터 분리
**이전**: 프로젝트 데이터가 Home.js 코드 안에 하드코딩
**이후**: `projects.json` 별도 파일로 분리
...
**효과**: 새 프로젝트 추가 시 코드 수정 없이 JSON 파일만 수정하면 됨

### 트러블슈팅: Hot Reload 문제
JSON 파일 수정 후 자동 반영 안 되는 경우 발생
→ 브라우저 강제 새로고침(Ctrl+Shift+R) 또는 개발 서버 재시작으로 해결

## 플랫폼화 로드맵
1. ✅ JSON 파일로 데이터 분리 (완료)
2. ⬜ 관리자 페이지 (웹에서 버튼으로 추가)
3. ⬜ 백엔드 + 데이터베이스 (진짜 플랫폼)

## 다음 할 것
- Node.js 백엔드 학습 시작
- 또는 관리자 페이지(폼 입력) 먼저 시도